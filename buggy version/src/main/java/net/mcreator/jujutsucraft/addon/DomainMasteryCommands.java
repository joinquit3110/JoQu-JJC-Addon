...
public class DomainMasteryCommands {
    ...
    private static int setForm(CommandContext<CommandSourceStack> ctx, int form) {
        ...
        DomainMasteryCommands.withData(player, data -> {
            boolean hasOpenBarrierAdvancement = DomainMasteryCommands.hasOpenBarrierAdvancement(player);
            data.setDomainTypeSelected(form, hasOpenBarrierAdvancement);
            int actualForm = Math.max(0, Math.min(2, data.getDomainTypeSelected()));
            String suffix = actualForm == form ? "" : " (requested " + names[form] + " was sanitized)";
            ((CommandSourceStack)ctx.getSource()).sendSuccess(() -> DomainMasteryCommands.gold("\u2726 Domain form set to " + names[actualForm] + suffix), true);
        });
        return 1;
    }
    ...
    private static int showInfo(CommandContext<CommandSourceStack> ctx) {
        ...
        DomainMasteryCommands.withData(player, data -> {
            ...
            double canonicalClashContribution = DomainMasteryCommands.computeCanonicalClashContribution(data);
            ((CommandSourceStack)ctx.getSource()).sendSuccess(() -> DomainMasteryCommands.white("  Clash Contribution: " + String.format("%+.1f", canonicalClashContribution)), false);
            ((CommandSourceStack)ctx.getSource()).sendSuccess(() -> Component.empty(), false);
    ...
    private static boolean hasOpenBarrierAdvancement(ServerPlayer player) {
        ...
        try {
            Advancement adv = player.server.getAdvancements().getAdvancement(new ResourceLocation("jujutsucraft", "mastery_open_barrier_type_domain"));
            if (adv == null) {
                return false;
            }
    ...
    private static double computeCanonicalClashContribution(DomainMasteryData data) {
        int clashPowerLevel = Math.max(0, data.getEffectiveLevel(DomainMasteryProperties.CLASH_POWER));
        int totalPowerPoints = data.getTotalPowerPoints();
        double baseClashPower = (double)clashPowerLevel * 0.6 * (1.0 + (double)data.getDomainMasteryLevel() * 0.03);
        double pointMultiplier = 1.0 + (double)totalPowerPoints * 0.04;
        return baseClashPower * pointMultiplier;
    }
