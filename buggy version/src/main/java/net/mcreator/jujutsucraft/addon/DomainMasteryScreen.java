...
/**
 * Full custom client GUI for domain mastery management. The screen renders animated panels, XP and form widgets, nine property cards, negative modify controls, and mutation locks during combat or active domains.
 */
// ===== SCREEN LIFECYCLE =====
...
extends Screen {
    ...
    private void refreshMutationLockState() {
        ...
        boolean inDomainClash = ClientPacketHandler.ClientDomainClashCache.isActive();
        ...
        LOGGER.info("[DomainClashDebug][MutationLock] player={} hasActiveDomain={} cacheClashActive={} cacheOpponents={} cacheCaster={} combatRemaining={}",
                mc.player.getScoreboardName(), hasActiveDomain, inDomainClash,
                ClientPacketHandler.ClientDomainClashCache.opponents.size(), ClientPacketHandler.ClientDomainClashCache.casterName,
                ClientPacketHandler.ClientCooldownCache.getRemaining(true));
        if (hasActiveDomain) {
    ...
    private void drawHeader(GuiGraphics graphics, int mx, int my) {
        ...
        if (clash > 0.0) {
            String cbText = "\u26a1 +" + Math.round(clash * 100.0) + "% Mastery Clash";
    ...
    private void drawPropertyTooltip(GuiGraphics g, int mx, int my) {
        ...
        if (!locked && prop.supportsNegativeModify()) {
            if (this.isNegativeProperty(this.hoveredPropIdx)) {
                int negativePoints = Math.abs(this.getEffectivePropertyLevel(prop));
                descBuilder.append("\nNegative Modify active \u2022 current ").append(prop.formatNegativeValue(negativePoints)).append("\nPress [+] to move back toward 0");
            } else if (this.getPropertyLevel(prop) > 0) {
                descBuilder.append("\nRefund to 0 before applying Negative Modify");
            } else if (this.getDomainMasteryLevel() < DomainMasteryData.NEGATIVE_UNLOCK_LEVEL) {
                descBuilder.append("\nNegative Modify unlocks at Domain Mastery Lv.").append(DomainMasteryData.NEGATIVE_UNLOCK_LEVEL);
            } else if (!this.canSetNegativeProperty(prop)) {
                descBuilder.append("\nAnother property is already negative");
            } else {
                descBuilder.append("\nPress [\u2212] to apply Negative Modify \u2022 each level grants +1 point and applies ").append(prop.formatNegativeValue(1));
            }
        }

        // Add power contribution breakdown from the point system
        Minecraft mc = Minecraft.getInstance();
        if (mc != null && mc.player != null) {
            mc.player.getCapability(DomainMasteryCapabilityProvider.DOMAIN_MASTERY_CAPABILITY, null).resolve()
                .ifPresent(data -> {
                    List<String> breakdown = data.getPowerContributionBreakdown();
    ...
    private double getClashBonus() {
        ...
        try {
            return mc.player.getCapability(DomainMasteryCapabilityProvider.DOMAIN_MASTERY_CAPABILITY, null).resolve()
                    .map(data -> data.getMasteryClashMultiplier() - 1.0)
                    .orElse(0.0);
        }
        catch (Exception e) {
            return Math.max(0, this.getDomainMasteryLevel()) * 0.04;
        }
    }
