...
/**
 * Persistent capability data model for addon domain mastery. It stores XP, form
 * unlocks, property levels, negative modify state, runtime scaling helpers,
 * client sync payloads, and NBT serialization.
 */
public class DomainMasteryData {
    ...
    // Legacy per-point debuff scalar retained for negative modify helper
    // calculations.
    public static final double NEGATIVE_DEBUFF_PER_POINT = 0.1;
    public static final double MASTERY_CLASH_BONUS_PER_LEVEL = 0.04;
    public static final double CLASH_RUNTIME_BONUS_PER_LEVEL = 0.06;
    public static final double CLASH_RUNTIME_NEGATIVE_PER_LEVEL = 0.03;
    public static final double POWER_POINT_MULTIPLIER_PER_POINT = 0.04;
    public static final double NEGATIVE_POWER_BONUS_PER_LEVEL = 0.06;
    public static final double BARRIER_REFINEMENT_BASE = 0.3;
    ...
    // Cached XP threshold for the next mastery level, used by the UI and sync
    // payloads.
    ...
    // Spendable property points earned from mastery progression and negative modify
    // tradeoffs.
    ...
    // Allocated level in barrier refinement.
    ...
    // Stored negative modify amount for the selected negative property, clamped
    // between 0 and -5.
    ...
    public void setDomainMasteryLevel(int level) {
        ...
        this.domainTypeSelected = DomainMasteryData.sanitizeFormSelection(this.domainTypeSelected,
    ...
    private void checkLevelUp() {
        while (this.domainMasteryLevel < 5
                && this.domainXP >= (double) DomainMasteryData.getXPRequiredForLevel(this.domainMasteryLevel + 1)) {
            ++this.domainMasteryLevel;
    ...
    private static int getXPRequiredForLevel(int level) {
        // These thresholds define the five-step addon mastery progression curve
        // described by the domain system design documents.
        return switch (level) {
            case 0 -> 0;
            case 1 -> 300;
            case 2 -> 700;
     ...
     * 
     * @param type                      type used by this method.
     * @param hasOpenBarrierAdvancement has open barrier advancement used by this
     *                                  method.
     */
    ...
    /**
     * Checks whether has open barrier advancement unlocked is true for the current
     * addon state.
     * 
     * @return true when has open barrier advancement unlocked succeeds; otherwise
     *         false.
     */
    ...
    public void setOpenBarrierAdvancementUnlocked(boolean unlocked) {
        ...
        this.domainTypeSelected = DomainMasteryData.sanitizeFormSelection(this.domainTypeSelected,
                this.domainMasteryLevel, unlocked);
    }
    ...
    /**
     * Clamps a form selection so players cannot keep forms that are not unlocked
     * for their current mastery state.
     * 
     * @param type         type used by this method.
     * @param masteryLevel level value used by this operation.
    ...
    public boolean canSetNegative(DomainMasteryProperties prop) {
        ...
        // Negative modify is intentionally locked behind max mastery so only fully
        // trained players can trade power in one category for points elsewhere.
    ...
    /**
     * Pushes the selected negative modify deeper, granting an extra property point
     * each time the penalty increases.
     * 
     * @param prop property identifier involved in this operation.
    ...
    /**
     * Reduces an existing negative modify by spending property points until the
     * penalty is fully cleared.
     * 
     * @param prop property identifier involved in this operation.
    ...
    public double getMasteryClashMultiplier() {
        return 1.0 + (double)this.domainMasteryLevel * MASTERY_CLASH_BONUS_PER_LEVEL;
    }

    public double getClashRuntimeMultiplier() {
        int baseLevel = this.getPropertyLevel(DomainMasteryProperties.CLASH_POWER);
        double positiveBonus = (double) baseLevel * CLASH_RUNTIME_BONUS_PER_LEVEL;
        double negativeReduction = 0.0;
        if (this.isNegativeProperty(DomainMasteryProperties.CLASH_POWER) && this.negativeLevel < 0) {
            negativeReduction = (double) Math.abs(this.negativeLevel) * CLASH_RUNTIME_NEGATIVE_PER_LEVEL;
        }
        return this.clampRuntimeMultiplier(1.0 + positiveBonus - negativeReduction);
    }
    ...
    private DomainMasteryProperties resolveNegativeProperty() {
        if (this.negativeProperty == null || this.negativeProperty.isEmpty()) {
            return null;
        }
        try {
            return DomainMasteryProperties.valueOf(this.negativeProperty);
        } catch (IllegalArgumentException ignored) {
            return null;
        }
    }
    ...
    // ===== NETWORK SYNC =====
    public void applySync(double xp, int level, int form, int points, int[] propLevels, String negativeProperty,
            int negativeLevel, boolean hasOpenBarrierAdvancement) {
        this.domainXP = xp;
    ...
    /**
     * Overwrites the local client-side copy of mastery data using values received
     * from the authoritative server capability.
     * 
     * @param xp                        xp used by this method.
     * @param level                     level value used by this operation.
     ...
     *                                  method.
     */
    public void applySync(double xp, int level, int form, int points, int[] propLevels,
    ...
    /**
     * Refunds every spent property point, clears the negative modify state, and
     * rebuilds the available point pool.
     */
    public void refundAllProperties() {
        ...
        // Negative modify grants bonus points up front, so a full refund must subtract
        // that borrowed value before restoring the pool.
    ...
    /**
     * Computes the total invested power points, counting only positive levels.
     * Negative modify levels do NOT contribute to power (they represent traded-away power).
     *
     * @return total positive points spent on CLASH_POWER, RADIUS_BOOST, BARRIER_REFINEMENT, and DURATION_EXTEND
     */
    public int getTotalPowerPoints() {
        int total = 0;
        // Only count positive levels - negative modify means you gave up power
        total += Math.max(0, this.getPropertyLevel(DomainMasteryProperties.CLASH_POWER));
        total += Math.max(0, this.getPropertyLevel(DomainMasteryProperties.RADIUS_BOOST));
        total += Math.max(0, this.getPropertyLevel(DomainMasteryProperties.BARRIER_REFINEMENT));
        total += Math.max(0, this.getPropertyLevel(DomainMasteryProperties.DURATION_EXTEND));
        return total;
    }

    /**
     * Computes the power multiplier bonus from negative modify states. Runtime behavior intentionally remains additive:
     * each traded-away negative level grants extra point-power multiplier.
     *
     * @return negative modify bonus value
     */
    private double getNegativeModifyPenalty() {
        double penalty = 0.0;
        // Only the property currently carrying negative modify applies penalty
        if (this.isNegativeProperty(DomainMasteryProperties.CLASH_POWER)) {
            penalty += Math.abs(this.negativeLevel) * NEGATIVE_POWER_BONUS_PER_LEVEL;
        }
        if (this.isNegativeProperty(DomainMasteryProperties.RADIUS_BOOST)) {
            penalty += Math.abs(this.negativeLevel) * NEGATIVE_POWER_BONUS_PER_LEVEL;
        }
        if (this.isNegativeProperty(DomainMasteryProperties.BARRIER_REFINEMENT)) {
            penalty += Math.abs(this.negativeLevel) * NEGATIVE_POWER_BONUS_PER_LEVEL;
        }
        if (this.isNegativeProperty(DomainMasteryProperties.DURATION_EXTEND)) {
            penalty += Math.abs(this.negativeLevel) * NEGATIVE_POWER_BONUS_PER_LEVEL;
        }
        return penalty;
    }
    ...
    /**
     * Returns the final point multiplier based on total invested power points
     * and any active negative modify tradeoff bonus.
     *
     * <p>Formula: (1.0 + points × {@link #POWER_POINT_MULTIPLIER_PER_POINT}) + negativeModifyBonus, clamped to [0.4, 2.0]</p>
     *
     * @return the final power point multiplier
     */
    public double getPowerPointMultiplier() {
        int powerPoints = this.getTotalPowerPoints();
        double baseMultiplier = 1.0 + powerPoints * POWER_POINT_MULTIPLIER_PER_POINT;
        double negativePenalty = this.getNegativeModifyPenalty();
        double finalMultiplier = baseMultiplier + negativePenalty;
        return Math.max(0.4, Math.min(2.0, finalMultiplier));
    }

    /**
     * Returns a detailed breakdown of power contributions for GUI display.
     * Shows each property's contribution and the total multiplier.
     *
     * @return list of description strings for GUI tooltips
     */
    public List<String> getPowerContributionBreakdown() {
        List<String> breakdown = new ArrayList<>();

        double masteryBonusPercent = (this.getMasteryClashMultiplier() - 1.0) * 100.0;
        if (masteryBonusPercent > 0.0) {
            breakdown.add(String.format(Locale.ROOT, "Mastery Lv bonus: +%.0f%% clash (Lv.%d)", masteryBonusPercent, this.domainMasteryLevel));
        }

        for (DomainMasteryProperties prop : new DomainMasteryProperties[] {
                DomainMasteryProperties.CLASH_POWER,
                DomainMasteryProperties.RADIUS_BOOST,
                DomainMasteryProperties.BARRIER_REFINEMENT,
                DomainMasteryProperties.DURATION_EXTEND
        }) {
            int level = this.getPropertyLevel(prop);
            if (level > 0) {
                breakdown.add(String.format(Locale.ROOT, "%s: %s (Lv.%d)", prop.name(), prop.formatLevelValue(level), level));
            }
        }

        if (this.hasNegativeModify()) {
            DomainMasteryProperties prop = this.resolveNegativeProperty();
            int negLevel = Math.abs(this.negativeLevel);
            String propName = prop != null ? prop.name() : this.negativeProperty;
            String negativeValue = prop != null ? prop.formatNegativeValue(negLevel) : String.format(Locale.ROOT, "-%d level", negLevel);
            breakdown.add(String.format(Locale.ROOT, "⚠ Negative %s: %s tradeoff, +%.2f point-power bonus (Lv.%d)",
                    propName, negativeValue, negLevel * NEGATIVE_POWER_BONUS_PER_LEVEL, negLevel));
        }

        double pointMultiplier = this.getPowerPointMultiplier();
        breakdown.add(String.format(Locale.ROOT, "Mastery Clash Multiplier: %.2fx", this.getMasteryClashMultiplier()));
        breakdown.add(String.format(Locale.ROOT, "Point Power Multiplier: %.2fx", pointMultiplier));
    ...
    public double getBarrierRefinementValue() {
        ...
        double base = BARRIER_REFINEMENT_BASE + baseLevel * BARRIER_REFINEMENT_PER_LEVEL;
        if (this.isNegativeProperty(DomainMasteryProperties.BARRIER_REFINEMENT) && this.negativeLevel < 0) {
            base -= Math.abs(this.negativeLevel) * BARRIER_REFINEMENT_PER_LEVEL;
        }
    ...
    public CompoundTag writeNBT() {
        ...
        nbt.putInt("jjkbrp_domain_type_selected",
                DomainMasteryData.sanitizeFormSelection(this.domainTypeSelected, this.domainMasteryLevel));
        ...
        // Both prefixed and legacy keys are written so older saved data and newer addon
        // revisions stay compatible.
    ...
    /**
     * Loads the mastery capability from persistent NBT while clamping invalid or
     * legacy values.
     * 
    ...
    public void readNBT(CompoundTag nbt) {
        ...
        this.domainTypeSelected = DomainMasteryData.sanitizeFormSelection(nbt.getInt("jjkbrp_domain_type_selected"),
                this.domainMasteryLevel);
        ...
        this.negativeProperty = nbt.contains("negativeProperty") ? nbt.getString("negativeProperty")
                : nbt.getString("jjkbrp_negative_property");
        this.negativeLevel = nbt.contains("negativeLevel") ? nbt.getInt("negativeLevel")
                : nbt.getInt("jjkbrp_negative_level");
        this.domainXPToNextLevel = DomainMasteryData.getXPRequiredForLevel(this.domainMasteryLevel + 1);
